...
@app.cell
def _(pd):
    ai_matching_df_cleaned = pd.read_csv('/home/akif/atBJobs/AiMatching/atbjobs-ai-matching-engine-v2/ai_matching_df_cleaned.csv')
...
@app.cell
def _(ai_matching_df_cleaned):
    ai_matching_df_cleaned['job_title']
    return


@app.cell
def _(ai_matching_df_cleaned):
    ai_matching_df_cleaned['job_skill_name']
    return
...
@app.cell
def _(data_batches):
    inputs_batch_0 = [[{"job_post": sample["job_post"]}] for sample in data_batches[0]]
    inputs_batch_1 = [[{"job_post": sample["job_post"]}] for sample in data_batches[1]]
    inputs_batch_2 = [[{"job_post": sample["job_post"]}] for sample in data_batches[2]]
    inputs_batch_3 = [[{"job_post": sample["job_post"]}] for sample in data_batches[3]]
    inputs_batch_4 = [[{"job_post": sample["job_post"]}] for sample in data_batches[4]]
    inputs_batch_5 = [[{"job_post": sample["job_post"]}] for sample in data_batches[5]]
    inputs_batch_6 = [[{"job_post": sample["job_post"]}] for sample in data_batches[6]]
    ...
    outputs_batch_0 = [{"resume": sample["resume"]} for sample in data_batches[0]]
    outputs_batch_1 = [{"resume": sample["resume"]} for sample in data_batches[1]]
    outputs_batch_2 = [{"resume": sample["resume"]} for sample in data_batches[2]]
    outputs_batch_3 = [{"resume": sample["resume"]} for sample in data_batches[3]]
    outputs_batch_4 = [{"resume": sample["resume"]} for sample in data_batches[4]]
    outputs_batch_5 = [{"resume": sample["resume"]} for sample in data_batches[5]]
    outputs_batch_6 = [{"resume": sample["resume"]} for sample in data_batches[6]]
...
def _(
    ...
    return (
        eval_inputs_batch_13,
        eval_inputs_batch_14,
        eval_inputs_batch_15,
        eval_inputs_batch_16,
        eval_inputs_batch_17,
        eval_inputs_batch_18,
        eval_inputs_batch_19,
    )


@app.cell
def _(eval_inputs_batch_14, judge, pd):
    results_14 = judge.batch_evaluate(eval_inputs_batch_14, save_results=False)
    ...
    results_dat_14= [
        ...
        for i, result in enumerate(results_14)
    ]

    df_14 = pd.DataFrame(results_dat_14)
    df_14.to_csv('data/ai_matching_annotation/resume_candidate/annotations_B12_1000.csv')
    return
